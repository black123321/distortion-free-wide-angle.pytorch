...
class Energy(nn.Module):
    ...
    def __init__(self, options, source_mesh, target_mesh, last_mash, correction_strength, box_masks, face_mask, Q, dtype=torch.float32):
        """
        ...
        self.opt = options
        self.last_mash = None
        if last_mash is not None:
            self.last_mash = last_mash
            self.last_mash = nn.Parameter(self.last_mash, requires_grad=False)
    ...
    def forward(self):
        ...
        # Temporal smoothness
        time_energy = 0
        if self.last_mash is not None:
            time_energy = (self.mesh - self.last_mash) ** 2
            time_energy = time_energy.mean()
        ...
        energy = self.opt["face_energy"] * face_energy \
                 ...
                 + self.opt["time_energy"] * time_energy
