...
from alignment_utils import four_vertex_crop, get_human_mask, match_feature_point

image_left = './images/left3.png'
image_right = './images/right3.png'
image_wide = './images/wide3.png'
...
A_l2w, mask1 = cv2.estimateAffinePartial2D(src_pts, dst_pts, method=cv2.RANSAC, ransacReprojThreshold=2.0)
...
A_r2w, _ = cv2.estimateAffinePartial2D(src_pts, dst_pts, method=cv2.RANSAC, ransacReprojThreshold=2.0)
...
h, w = wide.shape[:2]
left_warp = cv2.warpAffine(left, A_l2w, (w, h))
# left_warp = cv2.warpPerspective(left, H_l2w, (w, h))

cutting_left_warp = four_vertex_crop(left_warp)
...
h, w = wide.shape[:2]
right_warp = cv2.warpAffine(right, A_r2w, (w, h))
# right_warp = cv2.warpPerspective(right, H_r2w, (w, h))

cutting_right_warp = four_vertex_crop(right_warp)
